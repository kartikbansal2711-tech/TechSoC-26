//for running the program just enter the input it will work
// i am not sure which c++ 11 or 14 i am using but it should work
//i have used windows.h so it will only work for windows just use windows
...
vector<int> ccoo;
    ...
    for(int i=0;i<R;i++){
      ...
      intialpop+=cnt;
// printing generation 0 as not printed in loop
      if(n=="animate"){
        system("cls");
        cout<<"Generation: 0   Population:"<<intialpop<<endl;
        for(int i=0;i<R;i++){
        for(int k=0;k<C;k++){
            cout<<arr[i][k];
        }
...
// saving the generation into the 3d vector
 generations.push_back(arr);

 // method for checking oscillator(detects still life by period=1)
 for(int u=0;u<m;u++){
...
peak[m]=cnt3;
// printing each generation for 1 sec and the loop runs again
if(n=="animate"){
    system("cls");
            cout<<"Generation:"<<m<<"   "<<" Population: "<<cnt3<<endl;
